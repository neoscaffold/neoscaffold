  ...
  let NeoScaffoldStorage = (global.NeoScaffoldStorage = {
    ...
    async removeAllItems(storeName) {
      ...
      return new Promise((resolve, reject) => {
        ...
        request.onerror = (event) => {
          console.error(`Error removing all items from ${storeName}:`, event.target.error);
          reject(event.target.error);
        };
      });
    },
  ...
  let NeoScaffold = (global.NeoScaffold = {
    ...
    */
    ...
    async createGraph(canvasSelector, baseURL, sessionKey) {
      ...
      // Initialized state and tracking
      await scope.initializeState();
    ...
    async initializeState() {
      ...
      let workflowString, activeWorkflowChecksum;

      // check for the active workflow
      if (typeof localStorage !== 'undefined') {
        activeWorkflowChecksum = localStorage.getItem('neoscaffold_active_workflow_checksum');
      }

      // Load previous workflow
      let restored = false;
      try {
        if (activeWorkflowChecksum) {
          workflowString = await scope.storage.getItem('workflow', activeWorkflowChecksum);
        } else {
          workflowString = await scope.storage.getItem('workflow', 'workflow');
        }
      ...
      await scope.lazySetWorkflow(workflow);

      // Now done via keydown and mouseup events) Save current workflow automatically
      // setInterval(async () => {
    ...
    async lazySetWorkflow(workflow) {
      let checksumWorkflow;

      const currentWorkflow = workflow;
      const currentWorkflowJSON = JSON.stringify(currentWorkflow);

      // check if the storage doesn't have this checksum
      try {
        checksumWorkflow = await NeoScaffold.storage.getItem('workflow', currentWorkflow.checksum);
      } catch (error) {
    ...
    // see this: // https://github.com/swapnice/persona-avatar/blob/master/web/extensions/core/undoRedo.js
    async checkWorkflowState() {
      const currentWorkflow = await NeoScaffold.export();
      ...
      if (!activeChecksum || activeChecksum !== currentWorkflow.checksum) {
        ...
        NeoScaffold.undoStack.push(activeChecksum);
        NeoScaffold.redoStack = [];
      }
    },
    ...
    async undo() {
      if (NeoScaffold.undoStack.length > 0) {
        ...
        try {
          previousWorkflowJSON = await NeoScaffold.storage.getItem('workflow', previousWorkflowChecksum);
          previousWorkflow = JSON.parse(previousWorkflowJSON);
        } catch (error) {
          console.error("Error getting workflow from storage", error);
        }
        ...
        if (previousWorkflow) {
          ...
          NeoScaffold.getLocalStorage().setItem('neoscaffold_active_workflow_checksum', previousWorkflow.checksum);

          await NeoScaffold.import(previousWorkflow);
        }
      }
    },
    ...
    async redo() {
      if (NeoScaffold.redoStack.length > 0) {
        ...
        try {
          ...
          nextWorkflow = JSON.parse(nextWorkflowJSON);
        } catch (error) {
          console.error("Error getting workflow from storage", error);
        }
        ...
        if (nextWorkflow) {
          ...
          NeoScaffold.getLocalStorage().setItem('neoscaffold_active_workflow_checksum', nextWorkflow.checksum);

          await NeoScaffold.import(nextWorkflow);
        }
      }
    },
    ...
    buildWidgetOptions(widget, nodeObject) {
      ...
      const isIntegerNode = nodeClassName === 'nsInteger';
      const isIntegerNumberWidget =
        widgetKind === 'number' && (widget.integer === true || isIntegerNode);
      ...
      if (isIntegerNumberWidget) {
        // LiteGraph uses delta * 0.1 * step; step 10 => whole-number increments
    ...
    addKeyboardShortcuts(litegraphCanvas) {
      ...
      async function processKey(e) {
        ...
        if (e.type == 'keydown') {
          ...
          // undo ctrl + z
          if ((e.keyCode == 90 && e.ctrlKey && !e.shiftKey) || (e.key == 'z' && e.metaKey && !e.shiftKey)) {
            console.log("undo");
            NeoScaffold.undo();
            block_default = true;
          } else if ((e.keyCode == 90 && e.ctrlKey && e.shiftKey) || (e.key == 'z' && e.metaKey && e.shiftKey)) {
            // redo ctrl + shift + z
            console.log("redo");
            NeoScaffold.redo();
            block_default = true;
